...
const postTax = prices.map(num=>num*1.7)
...
// CODE HERE
const totalOrders = orders.map(order=>order.price+order.tax)
...
const bobsTotal = purchases.filter(order=>order.owner==='Bob').reduce((acc,curr)=>acc+curr.price,0)
